...
const findClosestCrossingPoint = (path1, path2) => {
  ...
  return path1.slice(0).reduce((accumulator, currentCoordinate, currentIndex, arr) => {
    ...
    if (!smallestCombinedSteps || path1StepsToCoordinate < smallestCombinedSteps) {
      const difference = smallestCombinedSteps
        ? smallestCombinedSteps - path1StepsToCoordinate
        : path2.length;
      const path2Index = path2
        .slice(0, difference)
        .findIndex(path2Coordinate => doCoordinatesMatch(currentCoordinate, path2Coordinate));
      ...
      if (
        ...
        return { crossingPoint: currentCoordinate, combinedSteps: thisCombinedSteps };
      }
    }
...
console.log('Combined steps to closest crossing point:', closestCrossingPoint.combinedSteps);
